...
- Do not add a participant-owned table without adding it to the centralized participant-owned-relations list (`src/services/participants/owned-relations.ts`, which holds platform_accounts and community_members). The merge relocation and the USER_HAS_DATA guard both iterate that list; a table left off it can be silently dropped when a ghost is merged (the participant_id foreign keys cascade on delete). The owned-relations catalog assertion (`test/db/owned-relations-catalog.test.ts`) fails loud if a participant_id foreign-key table is ever left off, but treat that as the backstop, not the reason to skip the discipline.
