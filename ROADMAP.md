...
Phases 1 (bootstrap), 2 (data-model foundation), and 3 (verification-driven claim-and-merge) have shipped; see the status snapshot and CHANGELOG.md. Ordering past phase 4 is provisional and is firmed one phase at a time. Phase numbering starts fresh for the rebuild and is kept stable, so shipped phases leave the arc but their numbers are not reused.

4. Community engagement core. Per-community XP and the network-level progression curve, message-activity ingestion and antispam, moderation and cases, and member join and leave events carrying account age and platform. This is the engine that produces signals. This phase must also extend the claim-and-merge: add community memberships and the XP and leveling tables to the centralized participant-owned-relations list, so a merge relocates them and the USER_HAS_DATA guard covers them (today the list holds only platform_accounts).
