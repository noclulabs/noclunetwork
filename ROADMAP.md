...
Phases 1 (bootstrap), 2 (data-model foundation), 3 (verification-driven claim-and-merge), 4a (community membership lifecycle), 4b-i (network engagement leveling), and 4c (moderation actions and sanction state) have shipped; see the status snapshot and CHANGELOG.md. With 4a, 4b-i, and 4c, phase 4 (the community engagement core) is complete, and phase 5 (the bridge) is the next major phase. Ordering past phase 4 is provisional and is firmed one phase at a time. Phase numbering starts fresh for the rebuild and is kept stable, so shipped phases leave the arc but their numbers are not reused.
