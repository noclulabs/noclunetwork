...
5. The bidirectional bridge to noCluID (cross-repo with noclulabs.com). The signal emitter (the append-only intake contract, the ledger's first real writer) and the scoped read contract (noCluID data for surfacing). On the noclulabs.com side this phase lands the intake receiver and the scoped read API as their own PRs.
